...

            {
                zoboAmounts.map((amount, index) => {
                    return (
                        ...
                        >
                            <span className={`text-xl transition-colors duration-300 font-bold ${amount.selected ? "text-white" : "text-red-700"}`}>{amount.amount}</span>
                        </div>
                    )
                })
            }
            <div className='w-14 h-14 flex items-center justify-center p-1 bg-white border-lg border border-zinc-300 rounded-xl '>
                <input value={amount} onChange={onInputChange} className='w-full h-full flex items-center justify-center text-center bg-transparent focus:outline-none outline-none' placeholder={toString(amount)} />
            </div>
        </div>
    )
}
