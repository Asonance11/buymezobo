...
export default function BankInfoStep() {
    ...
    const next = async () => {
        ...
        try {
            const response = await axios.get(`https://api.paystack.co/bank/resolve?account_number=${state.values.AccountNumber}&bank_code=${state.values.BankCode}`, {
                headers: {
                    Authorization: `Bearer ${CONFIG.paystack_key}`
                },
            });
    ...
    return (
        <section className='flex-col flex gap-1.5 lg:gap-3 p-3 lg:p-4 w-full md:w-2/3  mx-auto ' >
            ...
            <div className='flex flex-col gap-3'>
                ...
                <div className='p-2 bg-gray-50 rounded-lg  focus-within:border-2 focus-within:border-red-700'>
                    <input
                        ...
                        type="text" className='bg-transparent w-full text-sm focus:border-none outline-none' placeholder="Your Account Number" />
                </div>
            ...
            <div className='flex gap-3'>
                <Button onClick={skip} variant={"secondary"} className='text-sm lg:text-base' >skip</Button>
                <Button onClick={next} className='px-4 lg:px-8 text-sm lg:text-base'>Save bank details</Button>
            </div>
        </section>
    )
}
